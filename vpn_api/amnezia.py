...
Что делает адаптер сверх простого HTTP-клиента:
  ...
  1. Подставляет индивидуальные параметры мусорных пакетов (Jc/Jmin/Jmax)
     на каждую выдачу. Сервер их не разбирает, поэтому у каждого клиента
     может быть свой профиль — именно это мешает построить одно правило DPI
     на весь сервис. Общие параметры (S1-S4, H1-H4) задаются при создании
     интерфейса и здесь не трогаются: их изменение отключает всех разом.
...
class AmneziaWGAdapter(VPNPanelAdapter):
    ...
    async def create_client(self, name: str) -> IssuedClient:
        # Профиль мусорных пакетов генерируется на каждую выдачу: сервер их
        # игнорирует, поэтому у каждого клиента он свой.
