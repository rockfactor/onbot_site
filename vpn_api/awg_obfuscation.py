...
РАЗДЕЛЕНИЕ ПАРАМЕТРОВ — основа всей логики:
    ...
    Индивидуальные для клиента ПО ПРОТОКОЛУ (варьировать можно):
        Jc, Jmin, Jmax
        Управляют мусорными пакетами, которые клиент шлёт ДО рукопожатия.
        Сервер их просто игнорирует, поэтому у каждого клиента мог бы
        быть свой профиль — защита от универсального DPI-правила
        по размеру и числу пакетов.

Использование в адаптере панели AlexisHW:

    POST /api/servers                     -> obfuscation_params = ServerParams

ВАЖНО: передать ClientJunk в панель 1.8.5 нечем. Она копирует Jc/Jmin/Jmax
сервера во все клиентские конфигурации, а i_settings в
POST /api/servers/{sid}/clients задаёт только I1–I5 (произвольные пакеты
AWG 2.0) и к J-параметрам отношения не имеет. Поэтому generate_client_junk()
— заготовка: фактический профиль клиента задан параметрами сервера, и разнести
профили можно только по разным интерфейсам (мультисервер панели).

Отдельно: apply_i_settings=true без явного i1 подставляет захардкоженный
DEFAULT_I1, одинаковый у всех установок образа — готовая сигнатура для DPI.
Либо задавать i1 своим значением, либо не включать apply_i_settings.
...
def generate_client_junk() -> ClientJunk:
    ...
    ЗАГОТОВКА. Панель 1.8.5 не принимает эти значения на клиента (см.
    модульный докстринг), поэтому вызывать на каждую выдачу пока незачем:
    в конфиг всё равно попадут Jc/Jmin/Jmax сервера. Функция остаётся
    для прямой генерации конфигураций и для панели, где такой API появится.
    """
    jmin = _rand(20, 120)
