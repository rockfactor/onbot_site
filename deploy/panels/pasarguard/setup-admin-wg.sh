...
CONF
    ...
    #
    # MTU фиксируем в 1280 вместо умолчания 1420. Через туннель ходит только
    # управление, пропускная способность роли не играет, а вот надёжность —
    # играет: если на пути (мобильный оператор, PPPoE, ещё один туннель) MTU
    # меньше, крупные пакеты молча теряются. TCP при этом устанавливается —
    # SYN маленький, — и рвётся уже на флайте с TLS-сертификатом, который
    # занимает три с лишним килобайта. Диагностируется это тяжело, а 1280 —
    # минимальный MTU для IPv6, то есть значение, которое проходит везде.
    # Панель раздаёт своим клиентам ту же величину.
