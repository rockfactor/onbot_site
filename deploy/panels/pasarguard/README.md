...
Скрипт спросит порт, число устройств и нужен ли DNS-резолвер внутри туннеля
(про него — ниже, в разделе «Как открыть панель»; для этого сервера отвечаем
`y` и вводим `own.rockfactor.ru`). Дальше он сгенерирует ключи и создаст
...
### Что стоит в AllowedIPs и почему только подсеть

```
AllowedIPs = 10.10.0.0/24
```

Публичный адрес сервера — `159.194.204.110` — в этой строке стоять **не
должен**, хотя соблазн есть: панель открывается по имени, а имя резолвится
именно в него.

По тому же адресу лежит `Endpoint` туннеля. Клиент строит для каждого префикса
из `AllowedIPs` маршрут внутрь туннеля, и в маршрут `159.194.204.110/32`
попадают в том числе собственные UDP-пакеты WireGuard, адресованные
`159.194.204.110:51830`. Пакет, который обязан уйти наружу, чтобы туннель
поднялся, отправляется внутрь ещё не поднятого туннеля — петля маршрутизации.

`wg-quick` и десктопные клиенты на этом и останавливаются: рукопожатия нет,
панель не открывается. На Android и iOS такая конфигурация работает, но не
потому, что она верна: система сама исключает сокет туннеля из его маршрутов.
Проверено 23.08.2026 на сервере AWG-панели `ge01awg.rockfactor.ru` — туннель
там поднимается тем же скриптом, и с публичным адресом в `AllowedIPs` панель
не открывалась.

Это split-tunnel: через туннель идёт только `10.10.0.0/24`, остальной трафик
устройства — напрямую.
...
Строка обязательна. Умолчание клиента — 1420, и если на пути MTU меньше
(мобильный оператор, PPPoE, ещё один туннель), крупные пакеты теряются молча.
Диагностируется скверно: TCP-соединение **успешно устанавливается** —
`SYN` маленький, — а рвётся уже на флайте с TLS-сертификатом, который
занимает больше трёх килобайт. В браузере это `ERR_CONNECTION_ABORTED`,
в `curl -v` — обрыв сразу после `TLS handshake, Certificate (11)`, на сервере
косвенный признак — счётчик `sent` у пира в разы больше `received`
(ретрансмиссии сертификата).

На AWG-панели 23.08.2026 отказ был вызван именно этим: с исправленным
`AllowedIPs`, но без `MTU` панель не открывалась, добавление строки решило
вопрос. 1280 — минимальный MTU для IPv6, проходит везде; через админский
туннель ходит только управление, потерей пропускной способности можно
пренебречь.

`setup-admin-wg.sh` пишет строку сам. Конфигурации, созданные более ранней
версией скрипта, надо дополнить:
...
Панель закрыта вайтлистом и отвечает только на адрес внутри туннеля —
`10.10.0.1`. Сертификат при этом выписан на `own.rockfactor.ru`, поэтому
открывать по `https://10.10.0.1/` нельзя: браузер упрётся в несовпадение
имени. Значит, имя должно резолвиться в `10.10.0.1`, и туннель должен быть
подключён. Либо туннель обходится вовсе — см. способ 3.

**Способ 1 — резолвер внутри туннеля (рекомендуется).** `setup-admin-wg.sh`
поднимает `dnsmasq`, привязанный строго к интерфейсу `wg-admin`:

```
# /etc/dnsmasq.d/wg-admin-panel.conf
interface=wg-admin
bind-dynamic
except-interface=lo
address=/own.rockfactor.ru/10.10.0.1
no-resolv
server=1.1.1.1
server=8.8.8.8
```

Порт открывается тоже по интерфейсу, а не по номеру:
...
жертвы, то есть сервер начинает работать оружием в чужой атаке. Отсюда
`interface=` + `bind-dynamic` (сокет только на адресе туннеля, и он переживает
пересоздание интерфейса), `except-interface=lo` и правило `ufw` на интерфейс.
...
В клиентские конфигурации скрипт при этом дописывает строку:

```
DNS = 10.10.0.1
```

Работает на любом устройстве, включая телефон. Оговорка: пока туннель
подключён, `wg-quick` делает этот резолвер системным, то есть **весь** DNS
устройства идёт на сервер панели, который форвардит его наружу.

**Способ 2 — запись в hosts.** Если резолвер не поднимали, на каждом
устройстве вручную:

```bash
echo "10.10.0.1  own.rockfactor.ru" | sudo tee -a /etc/hosts
```

На телефоне так обычно нельзя — там остаётся только первый способ.

**Способ 3 — проброс порта по SSH, самый надёжный.** Панель слушает
`127.0.0.1:8000`, а SSH к серверу и так работает:

```bash
ssh -L 8443:127.0.0.1:8000 root@159.194.204.110
```

и в браузере — `http://127.0.0.1:8443/dashboard/`. В цепочке нет ни DNS,
ни сертификата, ни MTU, ни UDP — ломаться нечему, и работает оттуда, откуда
работает SSH. Туннель зависит от всего перечисленного сразу, поэтому держать
проброс основным способом, а туннель дополнительным — практичнее.

Вайтлист nginx при этом обходится: запрос приходит на `127.0.0.1:8000`
изнутри сервера. Доступ закрыт SSH-ключом плюс входом в панель — не слабее
ограничения по IP, а сильнее: ключ не подделывается, в отличие от адреса.

Две оговорки. На телефоне проброс порта практически неприменим — там нужен
туннель. И проверь при установке, работает ли вход по `http://127.0.0.1:8443`:
если панель ставит cookie сессии с флагом `Secure`, браузер не сохранит её
на нешифрованном origin, и тогда для входа понадобится туннель с TLS.
На AWG-панели проблемы нет — там Basic Auth.

### Если панель не открывается через туннель

| Симптом | Что это значит | Где смотреть |
|---|---|---|
| `DNS_PROBE_FINISHED_BAD_CONFIG`, имя не резолвится | до резолвера не дошли или спрашивают не его | `dig +short own.rockfactor.ru @10.10.0.1` на сервере; на Android — «Частный DNS» в настройках сети |
| TCP до `10.10.0.1:443` таймаутит | туннель не поднят или маршрут ушёл мимо | `wg show wg-admin` на сервере **при активном клиенте**; на устройстве `netstat -rn \| grep 10.10.0` |
| TCP установился, обрыв после `Certificate` в `curl -v`, в браузере `ERR_CONNECTION_ABORTED` | MTU: флайт с сертификатом больше 3 КБ. **Именно это ломало доступ на AWG-панели 23.08.2026** | строка `MTU = 1280` в `[Interface]` |

Блокировка WireGuard провайдером отличается тем, что при ней **рукопожатия
нет вообще** — в `wg show` у пира нет `latest handshake`, сколько бы клиент
ни был активен. Если рукопожатие есть и счётчики растут, причина в другом.
...
С подключённого устройства:

```bash
dig +short own.rockfactor.ru        # 10.10.0.1, если поднят резолвер
curl -sI https://own.rockfactor.ru/dashboard/ | head -1
```

Если `dig` отдаёт `159.194.204.110`, запрос уйдёт мимо туннеля и nginx ответит
`403`: имя резолвится не туда.
...
и создаёт владельца панели. Если страница не открывается, проверь, куда
резолвится имя: должно быть `10.10.0.1` (см. «Как открыть панель» в шаге 2).
