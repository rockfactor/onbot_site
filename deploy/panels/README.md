...
## Админский доступ к панели: проброс порта по SSH

Обе панели слушают loopback (`127.0.0.1:8080` у AWG, `127.0.0.1:8000`
у PasarGuard), поэтому открыть админку можно без туннеля:

```bash
ssh -L 8443:127.0.0.1:8080 root@144.31.246.234    # AWG
ssh -L 8443:127.0.0.1:8000 root@159.194.204.110   # PasarGuard
```

Дальше — `http://127.0.0.1:8443/` в браузере. В цепочке нет ни DNS,
ни сертификата, ни UDP, ни MTU — в отличие от админского туннеля, который
зависит от всего этого сразу. Доступ закрыт SSH-ключом плюс входом в панель.

Держите это основным способом, а туннель — дополнительным, для телефона,
где проброс порта неприменим. Подробности и разбор неисправностей туннеля —
в [`awg/README.md`](awg/README.md) и
[`pasarguard/README.md`](pasarguard/README.md).
